...
void Preprocess::avia_handler(const livox_ros_driver2::CustomMsg::ConstPtr &msg)
{
  ...
  {
    pl_buff[i].clear();
    pl_buff[i].reserve(plsize);// 每一个线scan保存的点云数量
  }
  ...
  {   // 分别对每个点云进行处理
    ...
    {// 只取线数在0~N_SCANS内并且回波次序为0或者1的点云
      ...
      {
        pl_full[i].x = msg->points[i].x;// 点x轴坐标
        pl_full[i].y = msg->points[i].y;// 点y轴坐标
        pl_full[i].z = msg->points[i].z;// 点z轴坐标
  ...
  {// 分别对每个点云进行处理
    ...
    { // 只取线数在0~N_SCANS内并且回波次序为0或者1的点云
      ...
      {
        valid_num ++;// 有效的点云数
        if (valid_num % point_filter_num == 0)// 等间隔降采样
        {
  ...
  std::cout<<"pl_surfpoints.size()"<<pl_surf.points.size()<<std::endl;
}
...
void Preprocess::PointCloud2_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  else
  {
    ...
    std::cout<< "pl_surf.points.size()"<<pl_surf.points.size()<<std::endl;
  }
  // pub_func(pl_surf, pub_full, msg->header.stamp);
  // pub_func(pl_surf, pub_corn, msg->header.stamp);
}
