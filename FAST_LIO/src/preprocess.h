...
// 枚举类型：表示支持的雷达类型
enum LID_TYPE{AVIA = 1, VELO16, OUST64,airsim}; //{1, 2, 3}
...
//   EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  void PointCloud2_handler(const sensor_msgs::PointCloud2::ConstPtr &msg);// 用于对velodyne激光雷达数据进行处理
  void give_feature(PointCloudXYZI &pl, vector<orgtype> &types);
  void pub_func(PointCloudXYZI &pl, const ros::Time &ct);
  int  plane_judge(const PointCloudXYZI &pl, vector<orgtype> &types, uint i, uint &i_nex, Eigen::Vector3d &curr_direct);// 没有用到
