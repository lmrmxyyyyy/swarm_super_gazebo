...
{
    MANUAL_HEAP q(2 * k_nearest);// 初始化自定义的大顶堆，容量为2*k_nearest
    q.clear();
    // 清空Point_Distance
    ...
    PointVector().swap(Nearest_Points); // 清空Nearest_Points和Point_Distance
    vector<float>().swap(Point_Distance);
    for (int i = 0; i < k_found; i++) // 存储
    {
...
int KD_TREE<PointType>::Add_Points(PointVector &PointToAdd, bool downsample_on)
{
    ...
    PointType downsample_result, mid_point;// 根据输入参数判断是否需要进行降采样
    bool downsample_switch = downsample_on && DOWNSAMPLE_SWITCH;
    float min_dist, tmp_dist;
    int tmp_counter = 0;// 遍历点集
    for (int i = 0; i < PointToAdd.size(); i++)
    {
        ...
        {   // 计算该点所属的体素
            ...
            min_dist = calc_dist(PointToAdd[i], mid_point); // 计算当前点于体素中心的距离
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
            {
                ...
                {
                    if (Downsample_Storage.size() > 0)// 删除体素内的原有点，然后插入当前点
            ...
            {// 需要重建树
                ...
                { // 如果近邻点不止1个，或者当前点与原有的点很接近
                    Operation_Logger_Type operation_delete, operation;// 记录待删除的点
                    ...
                    pthread_mutex_lock(&working_flag_mutex);
                    if (Downsample_Storage.size() > 0) // 删除体素内的原有点
                        Delete_by_range(&Root_Node, Box_of_Point, false, true); 
                    Add_by_point(&Root_Node, downsample_result, false, Root_Node->division_axis);// 添加点
                    tmp_counter++;
                    if (rebuild_flag)
                    {
                        pthread_mutex_lock(&rebuild_logger_mutex_lock);
        ...
        {// 不需要降采样
            ...
            {
                Operation_Logger_Type operation;// 记录待删除的点
...
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE **root, PointType point, bool allow_rebuild, int father_axis)
{
    ...
    if (((*root)->division_axis == 0 && point.x < (*root)->point.x) || ((*root)->division_axis == 1 && point.y < (*root)->point.y) || ((*root)->division_axis == 2 && point.z < (*root)->point.z))
    {
        ...
        {
            pthread_mutex_lock(&working_flag_mutex);// 线程互斥锁
...
{
    if (root == nullptr || root->tree_deleted)//如果当前节点为空或已被标记为删除，则返回。
        return;
    float cur_dist = calc_box_dist(root, point);
    float max_dist_sqr = max_dist * max_dist;
    if (cur_dist > max_dist_sqr)//计算当前节点代表的区域与查询点的最小距离。如果这个距离大于max_dist的平方，那么查询点不可能在这个节点的子树中找到更近的点，函数返回。
        return;
    int retval;
    if (root->need_push_down_to_left || root->need_push_down_to_right)
    {//如果当前节点有待推送的更新，则尝试获取互斥锁并执行Push_Down函数，将更新推送到子节点。
...
{// 该函数不需要递归，因为父节点的属性只与左右子节点相关
    ...
    else if (right_son_ptr != nullptr)// 左子树为空，右子树不为空
    {  // 左子树为空，右子树不为空
    ...
    root->radius_sq = x_L*x_L + y_L * y_L + z_L * z_L;//这是节点范围的一种度量，可以看作是包围盒的大小。
    // 让子节点知道自身父节点是谁
    if (left_son_ptr != nullptr)
        left_son_ptr->father_ptr = root;
    if (right_son_ptr != nullptr)
        right_son_ptr->father_ptr = root;
    if (root == Root_Node && root->TreeSize > 3)  // 如果当前节点为整个树的根节点，且树的节点数大于3
    {
        KD_TREE_NODE *son_ptr = root->left_son_ptr;
        if (son_ptr == nullptr)  // 如果当前节点为整个树的根节点，且树的节点数大于3
            son_ptr = root->right_son_ptr;
