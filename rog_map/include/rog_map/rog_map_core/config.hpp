        ...
        Config(const string &cfg_path, const string &name_space = "rog_map") {
            ...
            if (esdf_en) {
                if (temp_esdf_update_box.size() != 3) {
                    throw std::invalid_argument("Fix map origin size is not 3!");//esdf范围数组长度不是3
                } else {
            ...
            if (load_pcd_en) {//如果load_pcd_en，找到pcd_name的路径
                loader.LoadParam(name_space + "/pcd_name", pcd_name, string("map.pcd"));
                pcd_name = replaceCmakeRootDir(pcd_name);
            }

            loader.LoadParam(name_space + "/map_sliding/enable", map_sliding_en, true);//# 允许滑动更新
            loader.LoadParam(name_space + "/map_sliding/threshold", map_sliding_thresh, -1.0);//# 滑动更新阈值

            vector<double> temp_fix_origin;
            loader.LoadParam(name_space + "/fix_map_origin", temp_fix_origin, vector<double>{0, 0, 0}); //#固定地图中心（全局中心）
            ...
            loader.LoadParam(name_space + "/frontier_extraction_en", frontier_extraction_en, false);//边界提取

            loader.LoadParam(name_space + "/ros_callback/enable", ros_callback_en, false); // #回调函数
            loader.LoadParam(name_space + "/ros_callback/cloud_topic", cloud_topic, string("/cloud_registered"));// #点云输入
            // loader.LoadParam(name_space + "/ros_callback/cloud_f_topic", cloud_f_topic, string("/cam_cloud"));// #点云输入
            // loader.LoadParam(name_space + "/ros_callback/depth_topic", depth_topic, string("/realsense/depth_camera/depth/image_raw"));// #点云输入
            loader.LoadParam(name_space + "/ros_callback/odom_topic", odom_topic, string("/lidar_slam/odom"));  //#位姿输入
            loader.LoadParam(name_space + "/ros_callback/odom_timeout", odom_timeout, 0.05);//#位姿超时阈值


            loader.LoadParam(name_space + "/visualization/enable", visualization_en, false);//#可视化
            loader.LoadParam(name_space + "/visualization/use_dynamic_reconfigure", use_dynamic_reconfigure, false);//use rqt_reconfigure
            loader.LoadParam(name_space + "/visualization/pub_unknown_map_en", pub_unknown_map_en, false);  //The rate of visualization [Hz]
            ...
            loader.LoadParam(name_space + "/visualization/range", temp_vis_range, vector<double>{0, 0, 0});//# visualization range along the odom [m].
            ...
            loader.LoadParam(name_space + "/resolution", resolution, 0.1);  //#分辨率
            loader.LoadParam(name_space + "/inflation_resolution", inflation_resolution, 0.1);  //膨胀地图分辨率
            ...
            /* For unk inflation */
            loader.LoadParam(name_space + "/unk_inflation_en", unk_inflation_en, false);//#未知膨胀
            loader.LoadParam(name_space + "/unk_inflation_step", unk_inflation_step, 1);//#未知膨胀步数

            loader.LoadParam(name_space + "/inflation_step", inflation_step, 1);//#膨胀步数
            ...
            loader.LoadParam(name_space + "/intensity_thresh", intensity_thresh, -1);//#反射率阈值（滤掉）

            vector<double> temp_map_size;
            loader.LoadParam(name_space + "/map_size", temp_map_size, vector<double>{10, 10, 0});//#地图大小
            ...
            loader.LoadParam(name_space + "/point_filt_num", point_filt_num, 2);//滤点云The temporal downsample rate for input point cloud.
            if (point_filt_num <= 0) {
                std::cout <<  color_text::YELLOW << " -- [ROG] point_filt_num should be larger or equal than 1, it is set to 1 now."
                          << RESET << std::endl;
                point_filt_num = 1;
            }
            point_filt_num_init=point_filt_num;


            // raycasting
                //if disable, the map will only maintain occupied information, and all other grid
                // # will be considered as unknown.
            loader.LoadParam(name_space + "/raycasting/enable", raycasting_en, true);//#开启光线投射？
            loader.LoadParam(name_space + "/raycasting/batch_update_size", batch_update_size, 1);//#批量更新数量
            ...
            loader.LoadParam(name_space + "/raycasting/unk_thresh", unk_thresh, 0.70);//# 被视为空闲的未占用的 inf 地图网格的阈值。[0.0-1.0]
            ...
            loader.LoadParam(name_space + "/raycasting/ray_range", temp_ray_range, vector<double>{0.3, 10}); //#光线距离
            ...
            sqr_raycast_range_max = raycast_range_max * raycast_range_max;//平方
            sqr_raycast_range_min = raycast_range_min * raycast_range_min;
            vector<double> update_box;
            loader.LoadParam(name_space + "/raycasting/local_update_box", update_box, vector<double>{999, 999, 999});  //#局部更新范围
            ...
            loader.LoadParam(name_space + "/virtual_ground_height", virtual_ground_height, -0.1);  //#虚拟地面
            loader.LoadParam(name_space + "/virtual_ceil_height", virtual_ceil_height, -0.1); //#虚拟天花板

            resetMapSize();

            /// Probabilistic Update
#define logit(x) (log((x) / (1 - (x)))) //log
            l_hit = logit(p_hit);  //光线投射概率参数
            l_miss = logit(p_miss);
            l_min = logit(p_min);
            l_max = logit(p_max);
            l_occ = logit(p_occ);
            ...
            for (int dx = -inflation_step; dx <= inflation_step; dx++) {//膨胀球形邻居
            ...
            constexpr double max_search_dis = 2.0; //球形邻居 目标附近搜索用
        ...
        string odom_topic{}, cloud_topic{},cloud_f_topic{},depth_topic{};
        /* probability update */
        double raycast_range_min{}, raycast_range_max{};
        double sqr_raycast_range_min{}, sqr_raycast_range_max{};
        int point_filt_num{}, batch_update_size{},point_filt_num_init{};
