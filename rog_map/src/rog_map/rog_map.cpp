...
RobotState ROGMap::getRobotState() const {  //获取机器人状态
    return robot_state_;
}

void ROGMap::updateRobotState(const Pose& pose) {  //在updateMap 和 odomCallback 更新
    robot_state_.p = pose.first;
    robot_state_.q = pose.second;
    robot_state_.rcv_time = getSystemWalltimeNow();//return ros::Time::now().toSec();
