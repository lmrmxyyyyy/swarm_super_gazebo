...
Multi_UAV::Multi_UAV(const ros::NodeHandle &nh, const int & drone_id_) {
    ...
    LoadParam("multiuav/direct_teammate_assignment", direct_teammate_assignment, false);
    LoadParam("multiuav/direct_teammate_id", direct_teammate_id, -1);
    vector<double> direct_translation, direct_rotation_deg;
    LoadParam("multiuav/direct_teammate_translation", direct_translation, vector<double>{0.0, 0.0, 0.0});
    LoadParam("multiuav/direct_teammate_rotation_deg", direct_rotation_deg, vector<double>{0.0, 0.0, 0.0});
    if (direct_translation.size() == 3)
        direct_teammate_translation = V3D(direct_translation[0], direct_translation[1], direct_translation[2]);
    if (direct_rotation_deg.size() == 3)
        direct_teammate_rotation_deg = V3D(direct_rotation_deg[0], direct_rotation_deg[1], direct_rotation_deg[2]);

    if (direct_teammate_assignment &&
        (direct_teammate_id < 0 || direct_teammate_id >= MAX_UAV_NUM || direct_teammate_id == drone_id)) {
        ROS_ERROR_STREAM("Invalid direct teammate ID " << direct_teammate_id
                         << " for UAV" << drone_id << "; direct assignment disabled.");
        direct_teammate_assignment = false;
    }
    ...
    if (lidar_type == SIM) {
        ...
        topic_name_prefix = "uav" + SetString(drone_id) + "/";
        text_scale = 0.6;
        mesh_scale = 1.2;
    }
    ...
    const string teammate_assignment_topic = direct_teammate_assignment
            ? "configured_teammate_list" : "teammate_id_with_traj_matching";
    pubTeammateIdTrajMatching = nh_.advertise<swarm_msgs::ConnectedTeammateList>(
            "/" + topic_name_prefix + teammate_assignment_topic, 10);

    for (int i = 0; i < MAX_UAV_NUM; ++i) {
        if (direct_teammate_assignment && i != direct_teammate_id)
            continue;
    ...
    if (direct_teammate_assignment) {
        ROS_INFO_STREAM("UAV" << drone_id << " directly binds teammate UAV" << direct_teammate_id
                        << " (trajectory matching disabled), world translation = ["
                        << direct_teammate_translation.transpose() << "] m");
    }
}
...
void Multi_UAV::QuadstateCbk(const swarm_msgs::QuadStatePub::ConstPtr &msg) {
    ...
    if (id == drone_id)
        return;
    // In configured two-UAV mode, never create/read state for an unintended ID.
    if (direct_teammate_assignment && id != direct_teammate_id)
        return;
...
void Multi_UAV::UpdateFactorGraph(const bool &print_log) {
    if (direct_teammate_assignment || found_all_teammates)
        return;
...
bool Multi_UAV::DirectTeammateAssignmentEnabled() const {
    return direct_teammate_assignment;
}

void Multi_UAV::InitializeDirectTeammateTracker(StatesGroup &state_in, StatesGroup &state_prop,
                                                const double &lidar_end_time) {
    if (!direct_teammate_assignment || teammate_tracker.find(direct_teammate_id) != teammate_tracker.end())
        return;

    auto teammate_iter = teammates.find(direct_teammate_id);
    if (teammate_iter == teammates.end())
        return;

    const V3D direct_rotation_rad = direct_teammate_rotation_deg / 57.3;
    const M3D direct_rotation = EulerToRotM(direct_rotation_rad);
    const Teammate &teammate = teammate_iter->second;
    const V3D teammate_position = direct_rotation * teammate.teammate_state_temp.pos_end
                                  + direct_teammate_translation;
    const V3D teammate_velocity = direct_rotation * teammate.teammate_state_temp.vel;

    ESIKF drone_tracker(6, 3, 6);
    Matrix<double, 6, 1> initial_tracker_state;
    initial_tracker_state.head(3) = teammate_position;
    initial_tracker_state.tail(3) = teammate_velocity;
    drone_tracker.init(initial_tracker_state, lidar_end_time);
    teammate_tracker.insert(id2ekf(direct_teammate_id, drone_tracker));

    const double fixed_extrinsic_noise = 1e-6;
    state.global_extrinsic_rot[direct_teammate_id] = direct_rotation;
    state.global_extrinsic_trans[direct_teammate_id] = direct_teammate_translation;
    state.cov.block<6, 6>(18 + 6 * direct_teammate_id, 18 + 6 * direct_teammate_id) =
            fixed_extrinsic_noise * Matrix<double, 6, 6>::Identity();
    state_in = state;
    state_prop = state;
    found_all_teammates = true;

    ROS_INFO_STREAM("UAV" << drone_id << " initialized configured teammate UAV"
                    << direct_teammate_id << " without trajectory matching.");
}
...
void Multi_UAV::PublishQuadstate(const V3D &unbiased_gyr, const double &lidar_end_time, const double &first_lidar_time) {
    ...
    if (lidar_type == SIM)
        quadstate_msg_pub.child_frame_id = "uav" + SetString(drone_id) + "_aft_mapped";
    else
...
void Multi_UAV::PublishTeammateOdom(const double &lidar_end_time){
    ...
    for (auto iter = teammate_tracker.begin(); iter != teammate_tracker.end(); ++iter){
        ...
        TeammateOdom.child_frame_id = "uav" + SetString(teammate_id) + "_aft_mapped";
...
void Multi_UAV::PublishConnectedTeammateList(const double &lidar_end_time){
    ...
    // Publish the configured ID in direct mode; otherwise publish IDs found by trajectory matching.
    ...
    if (direct_teammate_assignment && teammate_tracker.find(direct_teammate_id) != teammate_tracker.end())
        list.connected_teammate_id.push_back(direct_teammate_id);
    else if (!direct_teammate_assignment) {
        for (int teammate_id : teammate_id_by_traj_matching)
            list.connected_teammate_id.push_back(teammate_id);
    }
