...
void Preprocess::process_cut_frame_livox(const livox_ros_driver2::CustomMsg::ConstPtr &msg,
    ...
    for (uint i = 1; i < plsize; i++) {
            ...
            (msg->points[i].tag & 0x30) == 0x00) //保留第0个回波
        {
            ...
            if(pl_full[i].curvature/1000 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << pl_full[i].curvature/1000 << RESET << endl;
                continue;
            }
...
//            double yaw_angle = atan2(pl_full[i].y, pl_full[i].x) * 57.3;
//            if((yaw_angle < -160 || yaw_angle > 160) && (cur_time - first_time) > 90)
//                continue;
...
void Preprocess::process_accumulate_frame_livox(const livox_ros_driver2::CustomMsg::ConstPtr &msg,
                                                PointCloudXYZI::Ptr &pcl_out, const double &first_lidar_head_time) {
    int plsize = msg->point_num;
    pl_surf.clear();
    pl_surf.reserve(plsize);
    pl_full.clear();
    ...
    for (uint i = 1; i < plsize; i++) {
            ...
            ((msg->points[i].tag & 0x30) == 0x00)) //保留第0个回波
        {
            ...
            //use curvature as time of each laser points，unit: ms
            pl_full[i].curvature =
                    msg->points[i].offset_time / float(1e6) + msg->header.stamp.toSec() - first_lidar_head_time;

            if(msg->points[i].offset_time * 1e-9 > 1.0/original_freq + 0.02){
                cout << BOLDRED << "offset time: " << msg->points[i].offset_time * 1e-9 << RESET << endl;
                continue;
            }
    ...
    if (feature_enabled) {
        for (uint i = 1; i < plsize; i++) {
            if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10) ||
                ...
                pl_full[i].intensity = msg->points[i].reflectivity;
                pl_full[i].curvature =
                        msg->points[i].offset_time / float(1e6); //use curvature as time of each laser points
        ...
        for (uint i = 1; i < plsize; i++) {
                ...
                (msg->points[i].tag & 0x30) == 0x00) //保留第0个回波
            {
                ...
                pl_full[i].intensity = msg->points[i].reflectivity;
                //use curvature as time of each laser points，unit: ms
                pl_full[i].curvature = msg->points[i].offset_time / float(1e6);
                if(pl_full[i].curvature/1000 > 1.0/original_freq + 0.02){
                    cout << BOLDRED << "offset time: " << pl_full[i].curvature/1000 << RESET << endl;
                    continue;
                }
