    ...
    if (imu_prop_enable && !prop_imu_buffer.empty()) {
        ...
        br1.sendTransform(
                tf::StampedTransform(transform, imu_prop_odom.header.stamp, topic_name_prefix + "world",
                                     "uav" + SetString(drone_id) + "_imu_propagation"));
    }
...
void publish_odometry(const ros::Publisher &pubLidarSlamOdom) {
    ...
    odomAftMapped.child_frame_id = "uav" + SetString(drone_id) + "_aft_mapped";
    ...
    br.sendTransform(
            tf::StampedTransform(transform, odomAftMapped.header.stamp, topic_name_prefix + "world",
                                 "uav" + SetString(drone_id) + "_aft_mapped"));
}
...
void TimerCbk(const ros::TimerEvent &e){
    if(LIO_pose.empty() || ground_truth.size() < 2)
        return;
...
//    fout_cov.open(DEBUG_FILE_DIR("quad" + SetString(drone_id) + "_cov.txt"), ios::out);
    ...
    const bool use_livox_custom_msg =
            (p_pre->lidar_type == AVIA || p_pre->lidar_type == SIM);
    ros::Subscriber sub_pcl = use_livox_custom_msg ? \
...
//                cout << "---------------------------------------" <<  RESET <<  endl;
            ...
            swarm->InitializeDirectTeammateTracker(state, state_propagat, lidar_end_time);
            swarm->state = state;
            ...
            // Temporary trackers only exist for automatic trajectory matching.
            if (!swarm->DirectTeammateAssignmentEnabled()) {
