...
namespace path_search {
    ...
    Astar::Astar(const std::string &cfg_path,
        ...
        cfg_ = PathSearchConfig(cfg_path); //加载a*地图参数
        cout << rog_map::GREEN << " -- [RM] Init Astar-map." << rog_map::RESET << endl;
        int map_buffer_size = cfg_.map_voxel_num(0) * cfg_.map_voxel_num(1) * cfg_.map_voxel_num(2);//地图开辟内存buffer大小
        grid_node_buffer_.resize(map_buffer_size); //GridNodePtr的vector
        for (auto &i: grid_node_buffer_) {  //初始化grid_node_buffer_
            i = new GridNode;
            i->rounds = 0;
        }
        ...
        for (int i = -test_num; i <= test_num; i++) {
            for (int j = -test_num; j <= test_num; j++) {
                for (int k = -test_num; k <= test_num; k++) {
                    ...
                    sorted_pts.push_back(delta);//Vec3i的vector
                }
            }
        }
        sort(sorted_pts.begin(), sorted_pts.end(),//从小到大排序，依据是每个点 到原点的欧几里得距离的平方
            ...
            for (int dx = -1; dx <= 1; dx++)
                for (int dy = -1; dy <= 1; dy++)
                    for (int dz = -1; dz <= 1; dz++) {
                        ...
                        if (!cfg_.allow_diag &&   //不允许对角线搜索，对角线continue
