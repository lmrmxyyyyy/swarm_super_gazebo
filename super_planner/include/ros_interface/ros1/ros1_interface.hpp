...
namespace ros_interface {
    ...
    public:
        ...
        void vizCommittedTraj(const geometry_utils::Trajectory &committed_traj,
            ...
            if (!visualization_en_) {//处理可视化的前提条件
                return;
            }
            ...
            Ros1Adapter::deleteAllMarkerArray(committed_traj_pub_);//清除之前的 Marker

            visualization_msgs::MarkerArray mkr_arr;

            double traj_dur = committed_traj.getTotalDuration();//计算轨迹总时长
            //如果存在备用轨迹
            if (backup_traj_start_TT > 0 && backup_traj_start_TT < traj_dur) {
                ...
                Ros1Adapter::addTrajectoryToMarkerArray(mkr_arr, committed_traj, "committed_exp", Color::Green(), 0.1,
                                                        true, false);
                committed_traj_pub_.publish(mkr_arr); //直接将整条轨迹 committed_traj 可视化，并且使用 绿色，宽度 0.1
            }
