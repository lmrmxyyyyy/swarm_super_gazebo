...
namespace super_planner {
    ...
    class SuperPlanner {
        ...
        bool dynamic_collision_en_{false};
        ...
        std::vector<ros::Subscriber> other_mpc_pred_subs_;
        std::map<int, MpcPredictionCache> other_mpc_predictions_;
        std::mutex other_mpc_pred_mutex_;
    ...
    private:
        ...
        void initializeDynamicCollisionAvoidance();
        void otherMpcPredCallback(const ius_msgs::TrajectoryConstPtr &msg, int other_id);
        int parseUavId(const std::string &name) const;
        bool sampleMpcPrediction(const ius_msgs::Trajectory &traj,
                                 const double &query_wt,
                                 Vec3f &pos) const;
        void buildTimeAwareDynamicHyperplanes(const PolytopeVec &sfc,
                                              const vec_E<Vec3f> &guide_path,
                                              const vector<double> &guide_stamp,
                                              const double &sfc_start_wt,
                                              std::vector<MatD4f> &dynamic_hplanes);
        void buildSwarmPredictions(std::vector<traj_opt::SwarmPrediction> &predictions);
