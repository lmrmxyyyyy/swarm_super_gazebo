...
int i_lid_after_miu_init=0;
bool get_relocal_odom=false;
// auto state_point;                           // 状态
...
void RGBpointBodyToWorld(PointType const * const pi, PointType * const po,auto debug_x)
{
    V3D p_body(pi->x, pi->y, pi->z);
    V3D p_global(debug_x.rot * (debug_x.offset_R_L_I*p_body + kf_output.x_.offset_T_L_I) + debug_x.pos);
...
void publish_frame_world(const ros::Publisher & pubLaserCloudFullRes)
{
    if (scan_pub_en)
    {
        ...
        // std::cout<<dense_pub_en<<std::endl;
         PointCloudXYZI::Ptr laserCloudFullRes(dense_pub_en ? feats_undistort : feats_down_body);
        int size = laserCloudFullRes->points.size();

        PointCloudXYZI::Ptr   laserCloudWorld(new PointCloudXYZI(size, 1));
        auto debug_x = kf_output.get_x(); //ycr
        ...
        {//点云从雷达坐标系转到世界
            RGBpointBodyToWorld(&laserCloudFullRes->points[i], \
                                &laserCloudWorld->points[i],debug_x);
        }
        sensor_msgs::PointCloud2 laserCloudmsg;
...
template<typename T>
void set_posestamp(T & out)//设置输出位姿
{
    if (!use_imu_as_input)
    {
...
void publish_odometry(const ros::Publisher & pubOdomAftMapped)//输出位姿
{
...
void publish_odometry_mavros(const ros::Publisher &vision_pub)//输出位姿
{
    odomAftMapped.header.frame_id = "camera_init";
    odomAftMapped.child_frame_id = "aft_mapped";
    if (publish_odometry_without_downsample)
    ...
    static tf::TransformBroadcaster brr, brrr,brrrr,brrrrr;
        tf::Transform                   transformm,transformm_body;
        tf::Quaternion                  qq;
        ...
        transformm_body.setOrigin(tf::Vector3(odomAftMapped.pose.pose.position.x, \
                                    odomAftMapped.pose.pose.position.y, \
                                    odomAftMapped.pose.pose.position.z));
        transformm_body.setRotation( q );
        ...
        odomAftMappedd.header.frame_id = "camera_init";
        odomAftMappedd.child_frame_id = "body";
        odomAftMappedd.header.stamp = ros::Time::now();
        brrrrr.sendTransform( tf::StampedTransform( transformm_body, odomAftMappedd.header.stamp, "camera_init", "body" ) );

}

void relocal_odom_cbk(const nav_msgs::OdometryConstPtr& odom_msg) 
{
    if (get_relocal_odom==true) return;
    auto debug_x = kf_output.get_x(); //ycr
    debug_x.pos[0] = odom_msg->pose.pose.position.x;
    debug_x.pos[1] = odom_msg->pose.pose.position.y;
    debug_x.pos[2] = odom_msg->pose.pose.position.z;
    // 提取四元数并转换为旋转矩阵
    Eigen::Quaterniond q(
    ...
    // 设置旋转矩阵
    debug_x.rot = q.toRotationMatrix();  // rot 是 Eigen::Matrix3d
    kf_output.change_x(debug_x);//ycr

    get_relocal_odom=true;
    ROS_INFO("Relocal_odom_cbk!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  ");
...
void publish_path(const ros::Publisher pubPath)
{
    ...
    static int jjj = 0;
    jjj++;
    if (jjj % 30 == 0) // if path is too large, the rvis will crash
    {
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub_relocal_odom = nh.subscribe(relocal_odom_topic, 200000, relocal_odom_cbk);
...
//------------------------------------------------------------------------------------------------------
    ...
    while (status)
    {
        ...
        if(sync_packages(Measures)) 
        {
            ...
            // state_point = kf_output.get_x(); //ycr
            ...
            // std::cout<<relocal_odom_topic<<std::endl;
            // continue;
            if (use_relocal_odom && (!get_relocal_odom))
            {
                continue;
            }
            else{
                i_lid_after_miu_init+=1;
                if(i_lid_after_miu_init>10000) i_lid_after_miu_init=100;
            }
            ...
            {     
                ...
                auto debug_x = kf_output.get_x(); //ycr
                // ROS_INFO("IMU Initial Done: Gravity111: %.4f %.4f %.4f ; state.pos: %.4f %.4f %.4f; ",\
                //     debug_x.gravity[0], debug_x.gravity[1], debug_x.gravity[2], debug_x.pos[0], debug_x.pos[1], debug_x.pos[2]);
                if (time_seq.size() > 0)
                {
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                 ...
                 if(i_lid_after_miu_init<20)
                    {
                        kf_output.change_x(debug_x);//ycr
                    }
                    debug_x = kf_output.get_x(); // 获取状态量
                    // ROS_INFO("IMU Initial Done: Gravity222: %.4f %.4f %.4f ; state.pos: %.4f %.4f %.4f;  ",\
                    // debug_x.gravity[0], debug_x.gravity[1], debug_x.gravity[2], debug_x.pos[0], debug_x.pos[1], debug_x.pos[2]);
