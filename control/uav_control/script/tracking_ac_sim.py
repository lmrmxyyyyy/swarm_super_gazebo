...
def publish_tarj_marker(points,msg,type):
    ...
    for i, p in enumerate(points):
        point_marker = Marker()
        point_marker.header.frame_id = "map"  # 替换为你的坐标系
...
def odom_cb(msg: Odometry):
    ...
    if True and mavros_state.mode != "OFFBOARD":   #关闭自动offboard
    ...
    if trajectory != None and len(trajectory._poss) != 0:
        ...
        # qw, qx, qy, qz = x[6+10:10+10]  # 四元数 [qw, qx, qy, qz]
        # # 从四元数转换为欧拉角
        # roll, pitch, yaw = tf.transformations.euler_from_quaternion([qx, qy, qz, qw])
        # u.orientation = Quaternion(*tf.transformations.quaternion_from_euler(roll, pitch, yaw))
        u.thrust = min(Tt/quad._a_z_max, 0.99)
        # u.thrust = 0
