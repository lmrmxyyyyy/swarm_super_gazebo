...
class QuadrotorSimpleModel(object):
    def __init__(self, cfg_f):
        ...
        if "drag_coeff" in self._cfg:
          self._D = np.diag(self._cfg["drag_coeff"])
        else:
           print("No drag coefficient specified in " + cfg_f)
        
        if "v_xy_max" in self._cfg:
          self._v_xy_max = self._cfg["v_xy_max"]
        else:
          self._v_xy_max = ca.inf
        if "v_z_max" in self._cfg:
          self._v_z_max = self._cfg["v_z_max"]
        else:
          self._v_z_max = ca.inf
...
class QuadrotorModel(object):
    def __init__(self, cfg_f):
        ...
        self._D_wind = np.diag([0.6, 0.6, 0.6])
        self._v_xy_max = ca.inf
        self._v_z_max = ca.inf
    ...
    def load(self, cfg_f):
      with open(cfg_f, 'r') as f:
        ...
        if "wind_coeff" in cfg:
          self._D_wind = np.diag(cfg["wind_coeff"])
        else:
           print("No self._D_wind coefficient specified in " + cfg_f)
        if "v_xy_max" in cfg:
...
#
    def dynamics(self):
        ...
        qw, qx, qy, qz = ca.SX.sym('qw'), ca.SX.sym('qx'), ca.SX.sym('qy'), ca.SX.sym('qz')
        wx, wy, wz = ca.SX.sym('wx'), ca.SX.sym('wy'), ca.SX.sym('wz')
        wind_x, wind_y, wind_z = ca.SX.sym('wind_x'), ca.SX.sym('wind_y'), ca.SX.sym('wind_z')
        T1, T2, T3, T4 = ca.SX.sym('T1'), ca.SX.sym('T2'), ca.SX.sym('T3'), ca.SX.sym('T4')

        taux = self._arm_l/np.sqrt(2)*(T1+T4-T2-T3)##前右下
        ...
        wind_drag =rotate_quat(ca.veccat(qw,qx,qy,qz) ,self._D_wind@rotate_quat(ca.veccat(qw,-qx,-qy,-qz), ca.veccat(wind_x,wind_y,wind_z)))
        X_dot = ca.vertcat(
            vx,
            vy,
            vz,
            2 * (qw * qy + qx * qz) * (-thrust/self._m) - fdrag[0]+wind_drag[0],
            2 * (qy * qz - qw * qx) * (-thrust/self._m) - fdrag[1]+wind_drag[1],
            (qw * qw - qx * qx - qy * qy + qz * qz) * (-thrust/self._m) + self._G - fdrag[2]+wind_drag[2],
            0.5 * (-wx * qx - wy * qy - wz * qz),
            0.5 * (wx * qw + wz * qy - wy * qz),
            0.5 * (wy * qw - wz * qx + wx * qz),
            0.5 * (wz * qw + wy * qx - wx * qy),
...
if __name__ == "__main__":
    quad = QuadrotorModel('/home/fractal/IntelligentUAVChampionshipBase/flm_dev/src/control/uav_control/script/function_model/quad/quad_robomaster.yaml')
